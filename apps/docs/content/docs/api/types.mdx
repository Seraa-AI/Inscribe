...
description: Core TypeScript types exported by @scrivr/core and @scrivr/react.
---
...
<Callout type="info">
  Scrivr uses 96 DPI as its reference density. 1 inch = 96 px, matching CSS `px` units at standard
...
The base type for all Scrivr extensions. Created via `Extension.create<Options>({ ... })`.
