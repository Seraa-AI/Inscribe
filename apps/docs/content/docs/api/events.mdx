...
description: Full signature reference for all Editor and useScrivrEditor event callbacks.
---
...
[package page](/packages/react) for framework-specific options.
...
`editor.subscribe()` is the underlying primitive that both `useScrivrEditor` and `<Scrivr />`
use internally.
