...
Scrivr exposes the current selection through `SelectionSnapshot` — a plain object you can
...
Use `editor.subscribe()` to react to every state change, or `useScrivrState` in React
